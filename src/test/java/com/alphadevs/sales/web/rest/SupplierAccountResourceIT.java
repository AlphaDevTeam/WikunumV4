...
public class SupplierAccountResourceIT {
    ...
    @Test
    @Transactional
    public void getAllSupplierAccountsByHistoryIsEqualToSomething() throws Exception {
        // Initialize the database
        supplierAccountRepository.saveAndFlush(supplierAccount);
        DocumentHistory history = DocumentHistoryResourceIT.createEntity(em);
        em.persist(history);
        em.flush();
        supplierAccount.setHistory(history);
        supplierAccountRepository.saveAndFlush(supplierAccount);
        Long historyId = history.getId();

        // Get all the supplierAccountList where history equals to historyId
        defaultSupplierAccountShouldBeFound("historyId.equals=" + historyId);

        // Get all the supplierAccountList where history equals to historyId + 1
        defaultSupplierAccountShouldNotBeFound("historyId.equals=" + (historyId + 1));
    }
