...
public class CashPaymentVoucherSupplierResourceIT {
    ...
    @Test
    @Transactional
    public void getAllCashPaymentVoucherSuppliersByHistoryIsEqualToSomething() throws Exception {
        // Initialize the database
        cashPaymentVoucherSupplierRepository.saveAndFlush(cashPaymentVoucherSupplier);
        DocumentHistory history = DocumentHistoryResourceIT.createEntity(em);
        em.persist(history);
        em.flush();
        cashPaymentVoucherSupplier.setHistory(history);
        cashPaymentVoucherSupplierRepository.saveAndFlush(cashPaymentVoucherSupplier);
        Long historyId = history.getId();

        // Get all the cashPaymentVoucherSupplierList where history equals to historyId
        defaultCashPaymentVoucherSupplierShouldBeFound("historyId.equals=" + historyId);

        // Get all the cashPaymentVoucherSupplierList where history equals to historyId + 1
        defaultCashPaymentVoucherSupplierShouldNotBeFound("historyId.equals=" + (historyId + 1));
    }
