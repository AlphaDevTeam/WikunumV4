...
public class UserPermissionsResourceIT {
    ...
    @Test
    @Transactional
    public void getAllUserPermissionsByHistoryIsEqualToSomething() throws Exception {
        // Initialize the database
        userPermissionsRepository.saveAndFlush(userPermissions);
        DocumentHistory history = DocumentHistoryResourceIT.createEntity(em);
        em.persist(history);
        em.flush();
        userPermissions.setHistory(history);
        userPermissionsRepository.saveAndFlush(userPermissions);
        Long historyId = history.getId();

        // Get all the userPermissionsList where history equals to historyId
        defaultUserPermissionsShouldBeFound("historyId.equals=" + historyId);

        // Get all the userPermissionsList where history equals to historyId + 1
        defaultUserPermissionsShouldNotBeFound("historyId.equals=" + (historyId + 1));
    }
