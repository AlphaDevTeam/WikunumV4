...
public class UnitOfMeasureResourceIT {
    ...
    @Test
    @Transactional
    public void getAllUnitOfMeasuresByHistoryIsEqualToSomething() throws Exception {
        // Initialize the database
        unitOfMeasureRepository.saveAndFlush(unitOfMeasure);
        DocumentHistory history = DocumentHistoryResourceIT.createEntity(em);
        em.persist(history);
        em.flush();
        unitOfMeasure.setHistory(history);
        unitOfMeasureRepository.saveAndFlush(unitOfMeasure);
        Long historyId = history.getId();

        // Get all the unitOfMeasureList where history equals to historyId
        defaultUnitOfMeasureShouldBeFound("historyId.equals=" + historyId);

        // Get all the unitOfMeasureList where history equals to historyId + 1
        defaultUnitOfMeasureShouldNotBeFound("historyId.equals=" + (historyId + 1));
    }
