...
public class DocumentTypeResourceIT {
    ...
    @Test
    @Transactional
    public void getAllDocumentTypesByHistoryIsEqualToSomething() throws Exception {
        // Initialize the database
        documentTypeRepository.saveAndFlush(documentType);
        DocumentHistory history = DocumentHistoryResourceIT.createEntity(em);
        em.persist(history);
        em.flush();
        documentType.setHistory(history);
        documentTypeRepository.saveAndFlush(documentType);
        Long historyId = history.getId();

        // Get all the documentTypeList where history equals to historyId
        defaultDocumentTypeShouldBeFound("historyId.equals=" + historyId);

        // Get all the documentTypeList where history equals to historyId + 1
        defaultDocumentTypeShouldNotBeFound("historyId.equals=" + (historyId + 1));
    }
