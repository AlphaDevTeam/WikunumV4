...
public class DocumentNumberConfigResourceIT {
    ...
    @Test
    @Transactional
    public void getAllDocumentNumberConfigsByHistoryIsEqualToSomething() throws Exception {
        // Initialize the database
        documentNumberConfigRepository.saveAndFlush(documentNumberConfig);
        DocumentHistory history = DocumentHistoryResourceIT.createEntity(em);
        em.persist(history);
        em.flush();
        documentNumberConfig.setHistory(history);
        documentNumberConfigRepository.saveAndFlush(documentNumberConfig);
        Long historyId = history.getId();

        // Get all the documentNumberConfigList where history equals to historyId
        defaultDocumentNumberConfigShouldBeFound("historyId.equals=" + historyId);

        // Get all the documentNumberConfigList where history equals to historyId + 1
        defaultDocumentNumberConfigShouldNotBeFound("historyId.equals=" + (historyId + 1));
    }
