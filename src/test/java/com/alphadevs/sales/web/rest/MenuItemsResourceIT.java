...
public class MenuItemsResourceIT {
    ...
    @Test
    @Transactional
    public void getAllMenuItemsByHistoryIsEqualToSomething() throws Exception {
        // Initialize the database
        menuItemsRepository.saveAndFlush(menuItems);
        DocumentHistory history = DocumentHistoryResourceIT.createEntity(em);
        em.persist(history);
        em.flush();
        menuItems.setHistory(history);
        menuItemsRepository.saveAndFlush(menuItems);
        Long historyId = history.getId();

        // Get all the menuItemsList where history equals to historyId
        defaultMenuItemsShouldBeFound("historyId.equals=" + historyId);

        // Get all the menuItemsList where history equals to historyId + 1
        defaultMenuItemsShouldNotBeFound("historyId.equals=" + (historyId + 1));
    }
