...
public class ItemBinCardResourceIT {
    ...
    @Test
    @Transactional
    public void getAllItemBinCardsByHistoryIsEqualToSomething() throws Exception {
        // Initialize the database
        itemBinCardRepository.saveAndFlush(itemBinCard);
        DocumentHistory history = DocumentHistoryResourceIT.createEntity(em);
        em.persist(history);
        em.flush();
        itemBinCard.setHistory(history);
        itemBinCardRepository.saveAndFlush(itemBinCard);
        Long historyId = history.getId();

        // Get all the itemBinCardList where history equals to historyId
        defaultItemBinCardShouldBeFound("historyId.equals=" + historyId);

        // Get all the itemBinCardList where history equals to historyId + 1
        defaultItemBinCardShouldNotBeFound("historyId.equals=" + (historyId + 1));
    }
