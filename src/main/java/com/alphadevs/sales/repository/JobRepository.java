...
public interface JobRepository extends JpaRepository<Job, Long>, JpaSpecificationExecutor<Job> {
    ...
    @Query(value = "select distinct job from Job job left join fetch job.assignedTos",
        countQuery = "select count(distinct job) from Job job")
    Page<Job> findAllWithEagerRelationships(Pageable pageable);

    @Query("select distinct job from Job job left join fetch job.assignedTos")
    List<Job> findAllWithEagerRelationships();

    @Query("select job from Job job left join fetch job.assignedTos where job.id =:id")
