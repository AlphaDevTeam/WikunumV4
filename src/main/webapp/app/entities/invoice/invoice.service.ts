...
export class InvoiceService {
  ...
  protected convertDateFromClient(invoice: IInvoice): IInvoice {
    const copy: IInvoice = Object.assign({}, invoice, {
      invDate: invoice.invDate && invoice.invDate.isValid() ? invoice.invDate.format(DATE_FORMAT) : undefined
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.invDate = res.body.invDate ? moment(res.body.invDate) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((invoice: IInvoice) => {
        invoice.invDate = invoice.invDate ? moment(invoice.invDate) : undefined;
      });
    }
    return res;
  }
