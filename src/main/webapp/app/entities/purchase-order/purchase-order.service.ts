...
export class PurchaseOrderService {
  ...
  protected convertDateFromClient(purchaseOrder: IPurchaseOrder): IPurchaseOrder {
    const copy: IPurchaseOrder = Object.assign({}, purchaseOrder, {
      poDate: purchaseOrder.poDate && purchaseOrder.poDate.isValid() ? purchaseOrder.poDate.format(DATE_FORMAT) : undefined,
      expireDate: purchaseOrder.expireDate && purchaseOrder.expireDate.isValid() ? purchaseOrder.expireDate.format(DATE_FORMAT) : undefined
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.poDate = res.body.poDate ? moment(res.body.poDate) : undefined;
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((purchaseOrder: IPurchaseOrder) => {
        purchaseOrder.poDate = purchaseOrder.poDate ? moment(purchaseOrder.poDate) : undefined;
        purchaseOrder.expireDate = purchaseOrder.expireDate ? moment(purchaseOrder.expireDate) : undefined;
      });
    }
    return res;
  }
