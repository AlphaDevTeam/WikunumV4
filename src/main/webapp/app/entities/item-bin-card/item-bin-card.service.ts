...
export class ItemBinCardService {
  ...
  protected convertDateFromClient(itemBinCard: IItemBinCard): IItemBinCard {
    const copy: IItemBinCard = Object.assign({}, itemBinCard, {
      transactionDate:
        itemBinCard.transactionDate && itemBinCard.transactionDate.isValid() ? itemBinCard.transactionDate.format(DATE_FORMAT) : undefined
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((itemBinCard: IItemBinCard) => {
        itemBinCard.transactionDate = itemBinCard.transactionDate ? moment(itemBinCard.transactionDate) : undefined;
      });
    }
    return res;
  }
