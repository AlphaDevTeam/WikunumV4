  ...
  ngOnInit(): void {
    this.activatedRoute.data.subscribe(({ cashPaymentVoucherCustomer }) => {
      ...
      this.documentHistoryService
        .query({ 'cashPaymentVoucherCustomerId.specified': 'false' })
        .pipe(
        ...
        .subscribe((resBody: IDocumentHistory[]) => {
          if (!cashPaymentVoucherCustomer.history || !cashPaymentVoucherCustomer.history.id) {
            this.histories = resBody;
          } else {
            this.documentHistoryService
              .find(cashPaymentVoucherCustomer.history.id)
              .pipe(
