...
export class PurchaseAccountBalanceComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    this.purchaseAccountBalanceService
      ...
      .subscribe(
        (res: HttpResponse<IPurchaseAccountBalance[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
        () => this.onError()
      );
  }
  ...
  registerChangeInPurchaseAccountBalances(): void {
    this.eventSubscriber = this.eventManager.subscribe('purchaseAccountBalanceListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IPurchaseAccountBalance[] | null, headers: HttpHeaders, page: number): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.router.navigate(['/purchase-account-balance'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
    ...
    this.purchaseAccountBalances = data ? data : [];
  }
