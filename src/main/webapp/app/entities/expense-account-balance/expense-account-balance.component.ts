...
export class ExpenseAccountBalanceComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    this.expenseAccountBalanceService
      ...
      .subscribe(
        (res: HttpResponse<IExpenseAccountBalance[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
        () => this.onError()
      );
  }
  ...
  registerChangeInExpenseAccountBalances(): void {
    this.eventSubscriber = this.eventManager.subscribe('expenseAccountBalanceListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IExpenseAccountBalance[] | null, headers: HttpHeaders, page: number): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.router.navigate(['/expense-account-balance'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
    ...
    this.expenseAccountBalances = data ? data : [];
  }
