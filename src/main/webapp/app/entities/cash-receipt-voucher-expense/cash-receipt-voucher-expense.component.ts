...
export class CashReceiptVoucherExpenseComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    this.cashReceiptVoucherExpenseService
      ...
      .subscribe(
        (res: HttpResponse<ICashReceiptVoucherExpense[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
        () => this.onError()
      );
  }
  ...
  registerChangeInCashReceiptVoucherExpenses(): void {
    this.eventSubscriber = this.eventManager.subscribe('cashReceiptVoucherExpenseListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: ICashReceiptVoucherExpense[] | null, headers: HttpHeaders, page: number): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.router.navigate(['/cash-receipt-voucher-expense'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
    ...
    this.cashReceiptVoucherExpenses = data ? data : [];
  }
